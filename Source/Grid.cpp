...
Grid::Grid(GridAndDetectorSystem mGridAndDetectorSystem)
    ...
    , leadStripsDistance(mGridAndDetectorSystem.leadStripsDistance)
    , leadStripsWidth(mGridAndDetectorSystem.leadStripsWidth)
    ...
    , materialGridStrip(mGridAndDetectorSystem.materialGridStrip)
    , uGridStrip(mGridAndDetectorSystem.uGridStrip / 1000)    // 1/mm --> 1/um
    , materialGridInterspacer(mGridAndDetectorSystem.materialGridInterspacer)
    , uInterspacer(mGridAndDetectorSystem.uInterspacer / 1000) // 1/mm --> 1/um  
...
Grid::Grid(float* gridOutput, GridAndDetectorSystem mGridAndDetectorSystem)
    ...
    , materialGridInterspacer(mGridAndDetectorSystem.materialGridInterspacer)
    , uInterspacer(mGridAndDetectorSystem.uInterspacer / 1000) // 1/mm --> 1/um 
    , rhoGS(mGridAndDetectorSystem.rhoGS)
    , grid(gridOutput)
{
...
{
    // 中间位置索引 discreteUnitNums / 2 - 1, 对于一半栅，空隙部分的一半索引 [discreteUnitNums / 2 - leadStripsDistance / 2, discreteUnitNums / 2 - 1]
    for (size_t i = discreteUnitNums / 2 - 1; i > discreteUnitNums / 2 - 1 - leadStripsDistance / 2; i--)
    {
        grid1D[i] = expf(-uInterspacer * unitH[i]);
    }

    // 余下的半栅赋值, 起始是 discreteUnitNums / 2 - leadStripsDistance / 2 - 1
    for (int i = discreteUnitNums / 2 - 1 - leadStripsDistance / 2; i >= 0; i -= gridPeriod)
    {
        for (int stripIndex = 0; stripIndex < leadStripsWidth; stripIndex++)
        {
            if (i - stripIndex < 0)
            {
                break;
            }
            grid1D[i - stripIndex] = expf(-uGridStrip * unitH[i - stripIndex]);    // 透过系数
        }
        for (int interspacerIndex = 0; interspacerIndex < leadStripsDistance; interspacerIndex++)
        {
            if (i - leadStripsWidth - interspacerIndex < 0)
            {
                break;
            }
            grid1D[i - leadStripsWidth - interspacerIndex] = expf(-uInterspacer * unitH[i]);
        }
...
void Grid::createDiscreteGrid_uEdO()
{
    ...
    // 除中点外，剩余部分的一半栅，空隙部分的一半索引 [discreteUnitNums / 2 - leadStripsDistance / 2, discreteUnitNums / 2 - 1]， leadStripsDistance离散后单元数 >= 3
    grid1D[discreteUnitNums / 2] = expf(-uInterspacer * unitH[discreteUnitNums / 2]);
    for (size_t i = discreteUnitNums / 2 - 1; i > discreteUnitNums / 2 - 1 - leadStripsDistance / 2; i--)
    {
        grid1D[i] = expf(-uInterspacer * unitH[i]); // expf(-uGridStrip * unitH[i]);
    }
...
void Grid::createDiscreteGrid_uOdE()
{
    ...
    // 对于一半栅，空隙部分的一半索引 [discreteUnitNums / 2 - leadStripsDistance / 2 + 1, discreteUnitNums / 2]
    for (size_t i = discreteUnitNums / 2; i > discreteUnitNums / 2 - leadStripsDistance / 2; i--)
    {
        grid1D[i] = expf(-uInterspacer * unitH[i]); // expf(-uGridStrip * unitH[i]);
    }

    // 余下的半栅赋值, 起始是 discreteUnitNums / 2 - leadStripsDistance / 2
    for (int i = discreteUnitNums / 2 - leadStripsDistance / 2; i >= 0; i -= gridPeriod)
    {
        for (int stripIndex = 0; stripIndex < leadStripsWidth; stripIndex++)
        {
            if (i - stripIndex < 0)
...
void Grid::createDiscreteGrid_uOdO()
{
    ...
    // 对于一半栅，空隙部分的一半索引 [discreteUnitNums / 2 - leadStripsDistance / 2, discreteUnitNums / 2 -1]， leadStripsDistance >= 3
    grid1D[discreteUnitNums / 2] = expf(-uInterspacer * unitH[discreteUnitNums / 2]);
    for (size_t i = discreteUnitNums / 2 -1; i > discreteUnitNums / 2 - 1 - leadStripsDistance / 2; i--)
    {
        grid1D[i] = expf(-uInterspacer * unitH[i]); // expf(-uGridStrip * unitH[i]);
...
{
	int x = leadStripsDistance;
	int y = leadStripsWidth;
	int z = dSize;
...
void Grid::defGrid()
{
    ...
    gridPeriod = leadStripsDistance + leadStripsWidth;
    ...
    /*char pathWrite[128];
    sprintf_s(pathWrite, sizeof(pathWrite), "GridImageData/Grid_dNums%dx%d_dSize%dum_S%s_IS%s_gridD%dum_gridd%dum_h%.2fmm_float_FD%.1fmm.raw", dNums, dNums, dSize, materialGridStrip, materialGridInterspacer, leadStripsDistance, leadStripsWidth, h/1000, FD/1000);
...
// 输入栅条衰减系数的单位为 1/mm.
void Grid::updataGrid(float uGridStrip)
{
    this->uGridStrip = uGridStrip / 1000;   // 1/mm -> 1/um 

    // 离散化单元总数是偶数, 间隙距离偶数um
    int UnitNumsFlag = discreteUnitNums % 2;
    int interSpaceFlag = leadStripsDistance % 2;
    switch (UnitNumsFlag)
    {
    case 0:
        if (interSpaceFlag == 0)
...
void Grid::showGridH()
{
    ...
    char pathWrite[128];
    sprintf_s(pathWrite, sizeof(pathWrite), "GridImageData/Grid_dNums%dx%d_dSize%dum_S%s_IS%s_gridD%dum_gridd%dum_h%.2fmm_float_FD%.1fmm_Hight%d.raw", dNums, dNums, dSize, materialGridStrip, materialGridInterspacer, leadStripsDistance, leadStripsWidth, h / 1000, FD / 1000, discreteUnitNums);
...
{
    char pathWrite[128];
    sprintf_s(pathWrite, sizeof(pathWrite), "AddIntensityImageData/Background(Grid)_dNums%dx%d_dSize%dum_gridD%dum_gridd%dum_h%.2f_Photon%d.raw", dNums, dNums, dSize, leadStripsDistance, leadStripsWidth, h, mI0Photons);
...
{
    char pathWrite[128];
    sprintf_s(pathWrite, sizeof(pathWrite), "AddIntensityImageData/AddGrid_dSize%dum_gridD%dum_gridd%dum_%s.raw", dSize, leadStripsDistance, leadStripsWidth, fileName);
...
{
    char pathWrite[256];
    sprintf_s(pathWrite, sizeof(pathWrite), "OutputResult/ForwardProjection/Grid/Grid_dNums%dx%d_dSize%dum_S%s_IS%s_gridD%dum_gridd%dum_h%.2fmm_FD%.1fmm_PeriodIndex.txt", dNums, dNums, dSize, materialGridStrip.c_str(), materialGridInterspacer.c_str(), leadStripsDistance, leadStripsWidth, h / 1000, FD / 1000);
...
void Grid::saveAsPeriodIndex(size_t* gridPeriodIndex)
{
    ...
    char pathWrite[128];
    sprintf_s(pathWrite, sizeof(pathWrite), "OutputResult/Grid/Grid_dNums%dx%d_dSize%dum_S%s_IS%s_gridD%dum_gridd%dum_h%.2fmm_FD%.1fmm_PeriodIndex.txt", dNums, dNums, dSize, materialGridStrip.c_str(), materialGridInterspacer.c_str(), leadStripsDistance, leadStripsWidth, h / 1000, FD / 1000);
